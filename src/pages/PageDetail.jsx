...
const COL_TOOLTIPS = {
  ...
  opportunityScore: 'اگر این ردیف فقط یک رتبه بالاتر برود، تقریباً این تعداد Click بیشتر می‌گیری (تخمینی، بر اساس رفتار معمول سایت خودت). فرض: impression ثابت می‌ماند.',
  intent: 'نیت کاربر بر اساس کلمات کلیدی موجود در کوئری — تقریبی است.',
  flagged: 'رتبه‌ات خوب است ولی Click تقریباً صفر — احتمالاً یک قابلیت گوگل (مثل پاسخ مستقیم یا AI Overview) Click را می‌خورد. SERP را دستی بررسی کن.',
};

const PAGE_SIZE = 150;

function n(num) { return num?.toLocaleString() ?? '—'; }
function fmtCTR(v) { return (v * 100).toFixed(2) + '%'; }
...
function oppInfo(row) {
  ...
  return [`+${n(row.opportunityScore)} Click`, 'badge-orange'];
}

const INTENT_CLS = { 'خرید': 'badge-orange', 'اطلاعاتی': 'badge-green', 'نامشخص': 'badge-gray' };

// نامشخص always last; asc: خرید first; desc: اطلاعاتی first
const INTENT_ORDER_ASC  = { 'خرید': 0, 'اطلاعاتی': 1, 'نامشخص': 2 };
const INTENT_ORDER_DESC = { 'اطلاعاتی': 0, 'خرید': 1, 'نامشخص': 2 };
...
function sortRows(rows, key, dir) {
  return [...rows].sort((a, b) => {
    ...
    if (key === 'intent') {
      const order = dir === 'asc' ? INTENT_ORDER_ASC : INTENT_ORDER_DESC;
...
const TableRow = memo(function TableRow({ row, isQuery, inList, onToggle }) {
  ...
  return (
    <tr>
      <td style={{ maxWidth: 260, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', direction: isQuery ? 'rtl' : 'ltr', textAlign: isQuery ? 'right' : 'left', fontSize: 12 }} title={row.label}>
        {row.label}
      </td>
      <td>{n(row.clicks)}</td>
      ...
      {isQuery && <td><span className={`badge ${INTENT_CLS[row.intent] ?? 'badge-gray'}`} style={{ fontSize: 11 }}>{row.intent}</span></td>}
      {isQuery && (
        <td style={{ textAlign: 'center' }}>
          {row.branded ? <span style={{ color: '#27ae60', fontWeight: 700 }}>✓</span> : <span style={{ color: '#ddd' }}>✗</span>}
        </td>
      )}
      <td>{row.flagged && <span className="badge badge-red" style={{ fontSize: 10, whiteSpace: 'nowrap' }}>بررسی SERP</span>}</td>
      <td style={{ textAlign: 'center' }}>
        <button
          onClick={onToggle}
          style={{
...
export default function PageDetail({
  ...
  useEffect(() => {
    if (initialSort) {
      ...
      setFlashCol(initialSort.key);
      setInitialSort(null);
      const t = setTimeout(() => setFlashCol(null), 1800);
  ...
  const TH = ({ col, label, tooltip }) => {
    ...
    const isFlashing = col === flashCol;
    return (
      <th onClick={() => handleSort(col)} style={{ position: 'relative', overflow: 'hidden' }}>
        {isFlashing && (
          <span style={{
            position: 'absolute', inset: 0,
            background: 'rgba(236,167,44,0.22)',
            animation: 'colFlash 1.6s ease forwards',
            pointerEvents: 'none',
          }} />
        )}
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2, position: 'relative' }}>
          <span style={{ opacity: isActive ? 1 : 0.3, marginRight: 2, fontSize: 11 }}>
        ...
        <div style={{ maxWidth: 1300, margin: '0 auto', padding: '24px 20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <div style={{ fontWeight: 700, fontSize: 18 }}>جدول جزئیات</div>
              <button
                onClick={() => setPage(2)}
                style={{
                  background: 'none',
            ...
            <div className="toggle-group">
              <button className={`toggle-btn${isQuery ? ' active' : ''}`} onClick={() => { setTableMode('queries'); setSortKey('opportunityScore'); setSortDir('desc'); }}>کوئری</button>
              <button className={`toggle-btn${!isQuery ? ' active' : ''}`} onClick={() => { setTableMode('pages'); setSortKey('opportunityScore'); setSortDir('desc'); }}>صفحه</button>
            </div>
          </div>
          ...
          <div className="table-wrap">
            <table className="data-table">
              ...
              <tbody>
                ...
                {sorted.length === 0 && (
                  <tr><td colSpan={20} style={{ textAlign: 'center', color: '#aaa', padding: '40px 0' }}>داده‌ای برای نمایش وجود ندارد</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          {visibleCount < sorted.length && (
            <div style={{ textAlign: 'center', marginTop: 16 }}>
              ...
              >
                ...
                <span style={{ opacity: 0.6, fontSize: 12, marginRight: 8 }}>({n(visibleCount)} از {n(sorted.length)})</span>
              </button>
            </div>
          )}
        </div>
